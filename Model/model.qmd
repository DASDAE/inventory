...
- A `distance_map` states at least one input axis, and every axis it states has one value per control point: they are the same points in different coordinates. A patch carrying more than one of those coordinates must agree with the map about all of them, or it contradicts the map about its own channels.
