...
`FiberArray` represents a distributed, fiber-based monitoring domain. Each `FiberArray` describes exactly one optical route: its `optical_paths` are non-overlapping time revisions of that route's description, and its `Acquisition` objects record the instrument configurations that produced data on it. Deployments with several concurrent routes (e.g., multiple arms radiating from one site) are represented as several `FiberArray`s, which can share physical resources such as interrogators and cables through `References`.
...
- Within a `FiberArray`, no more than one `OpticalPath` is valid for a given time: path validity windows must not overlap.
