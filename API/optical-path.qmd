...
`Patch.enrich` resolves the valid optical path from the patch `data_source_id` and patch time, mapping channels to optical distance internally (via the acquisition's `distance_map`) before projecting the path tracks:
