...
The same pattern applies to optical path revisions. Within a fiber array, optical paths are non-overlapping time revisions of one route, so a repair or break closes the current revision and adds a new one. For example, if a fiber breaks, one can discover the break by looking at continuous files, set the `end_time` of the current `OpticalPath`, and add the shortened path as the new valid revision.
...
closed_opath = opath.new(
    end_time=break_time,
    notes="Fiber break at 600 m.",
)
new_opath = (
    opath.split_at(break_distance)[0]
...
inventory_v2 = inventory.register_fiber_break(
    source_id="DAS.L001.01.RAW", time=break_time, distance=break_distance
)
```
...
`Inventory` is the single serialized source of metadata truth. Patches carry only a `data_source_id`; all richer context is resolved through the inventory on request, so metadata can be revised without rewriting data files.
