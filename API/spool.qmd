...
::: {.callout-note}
Of what this page describes, `attach_inventory`, `enrich`, `remove_inventory`, `unselect`, and selection on the *acquisition-level* facts an inventory states are implemented, as are the two smaller pieces which support them: `Inventory.get_names`, the accessor for the names an inventory could contribute, and the warning when a path segment overrides an attribute the file actually states. Still proposed, so that those code blocks are the intended API rather than one that runs today: `prune_to_inventory`, which is being renamed `conform_to_inventory` because subdividing a straddling patch can grow the spool rather than prune it, `split_by`, and selection on the coordinates an inventory defines *along the fiber* — tracks, annotation groups, and geometry axes.
:::
...
Selection on the acquisition-level facts — `gauge_length`, `interrogator.model`, and the rest of the `attrs` side — is whole-patch metadata work and is implemented. Precedence there is per row: a patch which states the name is judged by the index exactly as it would be without an inventory, and only the rows leaving it unstated are resolved, once per epoch rather than once per patch. So an archive whose headers are complete never consults the inventory, and one whose headers are empty pays for its epochs rather than its files. A patch the inventory does not describe, or describes twice because it straddles an epoch boundary, is not selected: select is a filter, and a patch with no single answer is no more selected than one which lacks the attr entirely.
