...
Start by transcribing the field notes into a table. This gives the inventory a single source for geometry and distance intervals. In practice this would be read from a CSV.
...
telemetry_cable = inv.Cable(
    ...
    fiber_count=1,
    description="Armored patch lead-in cable from interrogator.",
)
...
The optical path describes the components encountered by the light, along with geometry, coupling, and annotations that can later be projected onto patch coordinates.
...
telemetry_lead = inv.FiberSegment(
    ...
    fiber_standard="OS2",
    fiber_color="yellow",
    description="Not used for sensing.",
...
The 50 m telemetry lead-in has no survey, so it stays uncovered.
...
```python
trenched_zone = inv.OpticalPathAnnotation(
    start_distance=50.0, end_distance=470.0, group="zone", value="trenched"
)
loose_zone = inv.OpticalPathAnnotation(
    start_distance=470.0, end_distance=770.0, group="zone", value="loose"
)
```
...
optical_path = inv.OpticalPath(
    ...
    geometry=(east_trench, north_trench, surface_run),
    coupling=(trenched, loose),
    annotations=(trenched_zone, loose_zone),
).validate()
```
