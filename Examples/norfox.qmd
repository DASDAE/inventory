...
Each arm is a separate fiber array (see [Optical Paths And Fiber Arrays](#optical-paths-and-fiber-arrays) below), so each arm carries its own acquisition records. The acquisition records are cheap per-array metadata; the physical instruments they reference are shared resources, so no interrogator metadata is duplicated. Wavelength-specific settings can be added later if they are needed for an analysis, but they are not required for the structural inventory.

```python
def arm_acquisitions():
    common_fields = dict(
        location_code="",
        data_category="DAS",
    ...
    return (
        inv.Acquisition(code="FEBUS", interrogator=febus_a1r, **common_fields),
        inv.Acquisition(code="OPT1", interrogator=optdas_1, **common_fields),
        inv.Acquisition(code="OPT2", interrogator=optdas_2, **common_fields),
    )
```
...
### Optical Paths And Fiber Arrays

Each arm is a separate optical route, so each arm is a separate [FiberArray](../References/FiberArray.qmd) with exactly one valid [OpticalPath](../References/OpticalPath.qmd) at a time. That makes arm-level changes, outages, and repairs easy to represent as path revisions within the arm's fiber array, without touching the other arms. If a deployment instead spliced the arms in series into one optical route, it would be one `FiberArray` with a single long path and arm annotations.
...
for arm in ARMS:
    ...
    fiber_array = inv.FiberArray(
        ...
        acquisitions=arm_acquisitions(),
        optical_paths=(path,),
    )
    fiber_arrays.append(fiber_array)
```

A patch recorded by the Febus unit on arm A then resolves through the data source ID `NO.ARMA..FEBUS`.
...
The five arm fiber arrays sit beside the NORES stations under one network. The shared physical resources (interrogators, cables, pipes) live once in the inventory resources and are referenced from the arrays that use them.
...
- Each concurrent optical route is its own `FiberArray`; within an arm, the optical path can be revised over time without touching other arms.
- Shared physical instruments (the interrogators) are referenced from multiple fiber arrays without duplicating metadata.
