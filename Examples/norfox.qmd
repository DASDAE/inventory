...
Each arm is its own fiber array in this example, so each arm carries its own acquisition streams. Acquisition codes only need to be unique within a fiber array, so the same three codes recur on every arm — `NO.NFXA..FEBUS` and `NO.NFXB..FEBUS` are distinct `data_source_id`s. Wavelength-specific settings can be added later if they are needed for an analysis, but they are not required for the structural inventory.
...
def arm_couplings(arm):
    ...
    outbound = inv.CouplingCondition(distance=0.0, **kwargs)
    inbound = inv.CouplingCondition(distance=ARM_LENGTH, **kwargs)
    return outbound, inbound
...
Each arm is a separate [FiberArray](../References/FiberArray.qmd) carrying a single [OpticalPath](../References/OpticalPath.qmd). The arms are dissimilar routes — five directions radiating from one facility — so under the granularity rule they are five fiber arrays, one observing identity per route. (Similar co-located fibers, such as two fibers sharing one cable, would instead be location codes under a single array.) This also keeps arm-level changes cheap to represent: a break or repair on Arm C becomes an additive path epoch under `NFXC` while the other arms' metadata is untouched. A different implementation could use one compound path with arm annotations if the acquisition stream is stored as one continuous channel axis.
...
- Dissimilar concurrent routes are separate `FiberArray` identities; similar co-located fibers would be location codes under one array.
