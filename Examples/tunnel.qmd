...
The deployment uses two concurrent optical routes, so it is represented as two `FiberArray`s: `PDSS`, intended for a Febus G1 Brillouin DSS system, and `PDAS`, for a Sintela Onyxia DAS Rayleigh interrogator. Each fiber array has exactly one valid optical path at a time; the two routes run through different fibers of the same physical cables, which are shared resources referenced by both arrays.
