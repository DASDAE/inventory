...
- The affine channel map above (`start_distance` + `spatial_interval`) trusts the instrument's nominal numbers. When a tap test later shows the wellhead responding twelve channels late, replace `start_distance` with a measured [DistanceMap](../References/DistanceMap.qmd) — an in-place [correction](../Model/model.qmd).
